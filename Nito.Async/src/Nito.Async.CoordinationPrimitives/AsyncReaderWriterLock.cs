...
namespace Nito.Async
{
    ...
    public sealed class AsyncReaderWriterLock
    {
        ...
        private void ReleaseWaitersWhenCanceled(Task task)
        {
            task.ContinueWith(t =>
            {
                lock (SyncObject) { ReleaseWaiters(); }
        ...
        private void ReleaseWaiters()
        {
            if (_locksHeld == 0)
            {
                ...
                {
                    _upgradeableReaderKey = new UpgradeableReaderKey(this);
                    _upgradeableReaderQueue.Dequeue(_upgradeableReaderKey);
                    ++_locksHeld;
                }
            ...
            if (_locksHeld == 1)
            {
                ...
                {
                    _upgradeReaderQueue.Dequeue(_upgradeableReaderKey._cachedUpgradeKeyTask.Result);
                    _locksHeld = -1;
                }
            }
        ...
        internal void ReleaseUpgradeableReaderLock(Task upgrade)
        {
            ...
            {
                if (upgrade != null)
                    _upgradeReaderQueue.TryCancel(upgrade, new CancellationToken(true));
        ...
        public sealed class UpgradeableReaderKey : IDisposable
        {
            ...
            {
                _asyncReaderWriterLock = asyncReaderWriterLock;
                _cachedUpgradeKeyTask = Task.FromResult<IDisposable>(new UpgradeKey(this));
            }
            ...
            private void Downgrade()
            {
                lock (_asyncReaderWriterLock.SyncObject)
                {
                    _asyncReaderWriterLock.Downgrade();
                    _upgrade = null;
                }
            }
